...
def create_admin(request):
    ...
    if not settings.DEBUG:
        ...
        return HttpResponse("This setup endpoint is disabled in production", status=403)
    ...
    if not User.objects.filter(username='admin').exists():
        user = User.objects.create_user(
            username='Sebe_TV',
            email='sebetvbusiness@gmail.com',  # Change this
        ...
        # Display the password only once
        return HttpResponse(f"Admin created with temporary password: {temp_password}<br>"
        ...
        return HttpResponse(f"Admin password reset to: {temp_password}<br>"
                           f"Please login and change it immediately.")