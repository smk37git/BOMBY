...
# Add this new view to ensure CSRF cookie is set
@ensure_csrf_cookie
...
def firebase_login(request):
    ...
    if request.method == 'POST':
        try:
            ...
            logger.debug(f"Firebase login request: {request.body[:200]}...")
            
            data = json.loads(request.body)
            id_token = data.get('idToken')
            
            if not id_token:
                logger.error("No idToken provided in request")
                return JsonResponse({
                    'success': False,
                    'error': 'No authentication token provided'
                }, status=400)
            
            logger.debug(f"Token received, length: {len(id_token)}")
            ...
            if user:
                ...
                logger.warning("Firebase authentication failed to return a user")
                return JsonResponse({
                    'success': False,
            ...
            logger.exception(f"Error in firebase_login: {str(e)}")
            return JsonResponse({
                'success': False,
                'error': str(e)
            }, status=500)
...
def firebase_signup(request):
    """Handle Firebase signup and create Django user"""
    if request.method == 'POST':
        try:
            # Log the request for debugging
            logger.debug(f"Firebase signup request: {request.body[:200]}...")
            ...
            # Create or get user with Firebase token
            user = authenticate(request=request, firebase_token=id_token)
            ...
            if user:
                ...
                logger.info(f"Successfully created/logged in user: {user.username}")
                return JsonResponse({
                    'success': True,
                    'user': {
                        'id': user.id,
                ...
                return JsonResponse({
                    'success': False,
                    'error': 'Failed to create user with Firebase token'
                }, status=401)
            ...
            logger.exception(f"Error in firebase_signup: {str(e)}")
            return JsonResponse({
                'success': False,
                'error': str(e)
            }, status=500)
...
def firebase_config(request):
    ...
    try:
        ...
        logger.exception(f"Error returning Firebase config: {str(e)}")
        return JsonResponse({
