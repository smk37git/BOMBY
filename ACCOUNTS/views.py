...
# Optional: Content moderation function for AWS Rekognition
def moderate_image_content(image_file):
    """
    Uses AWS Rekognition to detect inappropriate content
    ...
    try:
        ...
        for label in response['ModerationLabels']:
            if label['Confidence'] > 70:  # Higher confidence threshold for rejection
                explicit_categories.append(label['Name'])
        
        is_safe = len(explicit_categories) == 0
        return is_safe, explicit_categories
    except Exception as e:
        # Log the error but don't block upload on service failure
        print(f"Error in content moderation: {str(e)}")
        return True, []  # Allow upload if service fails