...
def activate_fuze_user(user):
    """Mark user as Fuze user on first app login"""
    if not user.fuze_activated:
        user.fuze_activated = True
        user.fuze_first_login = timezone.now()
    user.fuze_last_active = timezone.now()
    user.fuze_total_sessions += 1
    user.save()
...
def send_widget_refresh(user_id: int, widget_type: str, platform: str = None):
    ...
    Replaces the long if/elif chain in fuze_save_widget.
    """
