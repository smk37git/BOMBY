...
function connectWS() {{
    if (config.show_twitch && connectedPlatforms.includes('twitch')) {{
        connections.push(createWS(`wss://bomby.us/ws/fuze-alerts/${{userId}}/twitch/?token=${{widgetToken}}`));
    }}
    if (config.show_youtube && connectedPlatforms.includes('youtube')) {{
        connections.push(createWS(`wss://bomby.us/ws/fuze-alerts/${{userId}}/youtube/?token=${{widgetToken}}`));
    }}
    if (config.show_kick && connectedPlatforms.includes('kick')) {{
        connections.push(createWS(`wss://bomby.us/ws/fuze-alerts/${{userId}}/kick/?token=${{widgetToken}}`));
    }}
    if (config.show_facebook && connectedPlatforms.includes('facebook')) {{
        connections.push(createWS(`wss://bomby.us/ws/fuze-alerts/${{userId}}/facebook/?token=${{widgetToken}}`));
    }}
    if (config.show_tiktok && connectedPlatforms.includes('tiktok')) {{
        connections.push(createWS(`wss://bomby.us/ws/fuze-alerts/${{userId}}/tiktok/?token=${{widgetToken}}`));
    }}
    if (config.show_donations) {{
        connections.push(createWS(`wss://bomby.us/ws/fuze-donations/${{userId}}/?token=${{widgetToken}}`));
    }}
}}
...
if (config.show_youtube && connectedPlatforms.includes('youtube')) {{
    fetch(`https://bomby.us/fuze/youtube/start/${{userId}}?token=${{widgetToken}}`).catch(() => {{}});
    setInterval(() => fetch(`https://bomby.us/fuze/youtube/start/${{userId}}?token=${{widgetToken}}`).catch(() => {{}}), 300000);
}}
if (config.show_kick && connectedPlatforms.includes('kick')) {{
    fetch(`https://bomby.us/fuze/kick/start/${{userId}}?token=${{widgetToken}}`).catch(() => {{}});
    setInterval(() => fetch(`https://bomby.us/fuze/kick/start/${{userId}}?token=${{widgetToken}}`).catch(() => {{}}), 300000);
}}
if (config.show_facebook && connectedPlatforms.includes('facebook')) {{
    fetch(`https://bomby.us/fuze/facebook/start/${{userId}}?token=${{widgetToken}}`).catch(() => {{}});
    setInterval(() => fetch(`https://bomby.us/fuze/facebook/start/${{userId}}?token=${{widgetToken}}`).catch(() => {{}}), 300000);
}}
if (config.show_tiktok && connectedPlatforms.includes('tiktok')) {{
    fetch(`https://bomby.us/fuze/tiktok/start/${{userId}}?token=${{widgetToken}}`).catch(() => {{}});
    setInterval(() => fetch(`https://bomby.us/fuze/tiktok/start/${{userId}}?token=${{widgetToken}}`).catch(() => {{}}), 300000);
}}
...
fetch(`https://bomby.us/fuze/widgets/events/config/${{userId}}/all?token=${{widgetToken}}&t=${{Date.now()}}`)
    .then(r => r.json())
    .then(data => {{
...
function handleMessage(e) {{
    ...
    if (platform === 'donation') {{
        if (!config.show_donations) return;
    }} else if (!config[`show_${{platform}}`]) {{
        return;
    }}
...
function processQueue() {{
    ...
    const layout = alertConfig.layout || 'image_above';
    ...
    const animation = alertConfig.alert_animation || 'fade';
    alert.style.animation = `${{animation}}In 0.5s ease-out forwards`;
    
    if (alertConfig.image_url) {{
    ...
    text.style.fontSize = (alertConfig.font_size || 32) + 'px';
    text.style.fontWeight = alertConfig.font_weight || 'normal';
    text.style.fontFamily = alertConfig.font_family || 'Arial';
    text.style.color = alertConfig.text_color || '#FFFFFF';
    
    if (alertConfig.text_shadow) {{
        text.style.textShadow = '2px 2px 4px rgba(0,0,0,0.8)';
    }}
    
    const eventData = data.event_data || {{}};
    let message = alertConfig.message_template || '{{name}} just followed!';
    ...
    if (alertConfig.text_animation && alertConfig.text_animation !== 'none') {{
        text.style.animation = `${{alertConfig.text_animation}} 1s ease-in-out infinite`;
    }}
    
    if (layout === 'text_over_image' && alertConfig.image_url) {{
    ...
    if (data.event_type === 'donation' && eventData.message && eventData.message.trim()) {{
        ...
        msgEl.style.fontSize = Math.max(14, (alertConfig.font_size || 32) * 0.6) + 'px';
        msgEl.style.fontFamily = alertConfig.font_family || 'Arial';
        msgEl.style.color = alertConfig.text_color || '#FFFFFF';
    ...
    // TTS handling - disabled if ?notts=1 parameter present (for Fuze preview)
    ...
    if (!ttsDisabled && alertConfig.tts_enabled && ['donation', 'bits', 'superchat', 'stars'].includes(data.event_type)) {{
        ...
        const ttsText = (alertConfig.tts_template || defaultTtsTemplates[data.event_type] || '{{name}} donated {{amount}}. {{message}}')
...
fetch(`https://bomby.us/fuze/twitch-chat/start/${{userId}}?token=${{widgetToken}}`).catch(() => {{}});
fetch(`https://bomby.us/fuze/kick-chat/start/${{userId}}?token=${{widgetToken}}`).catch(() => {{}});
fetch(`https://bomby.us/fuze/facebook-chat/start/${{userId}}?token=${{widgetToken}}`).catch(() => {{}});
fetch(`https://bomby.us/fuze/youtube/start/${{userId}}?token=${{widgetToken}}`).catch(() => {{}});
fetch(`https://bomby.us/fuze/kick/start/${{userId}}?token=${{widgetToken}}`).catch(() => {{}});
fetch(`https://bomby.us/fuze/tiktok/start/${{userId}}?token=${{widgetToken}}`).catch(() => {{}});
...
let ws;
function connectWS() {{
    ws = new WebSocket('wss://bomby.us/ws/fuze-chat/' + userId + '/?token=' + widgetToken);
    ws.onmessage = handleMessage;
...
PLATFORM_ICONS = {{
    'twitch': 'https://storage.googleapis.com/fuze-public/widget-icons/twitch-platform.png',
    'youtube': 'https://storage.googleapis.com/fuze-public/widget-icons/youtube-platform.png',
    'kick': 'https://storage.googleapis.com/fuze-public/widget-icons/kick-platform.png',
    'facebook': 'https://storage.googleapis.com/fuze-public/widget-icons/facebook-platform.png',
    'tiktok': 'https://storage.googleapis.com/fuze-public/widget-icons/tiktok-platform.png',
}}

const BADGE_URLS = {{
    twitch: {{
        'broadcaster': 'https://storage.googleapis.com/fuze-public/widget-icons/twitch-broadcaster.png',
        'moderator': 'https://storage.googleapis.com/fuze-public/widget-icons/twitch-moderator.png',
        'subscriber': 'https://storage.googleapis.com/fuze-public/widget-icons/twitch-subscriber.png',
        'vip': 'https://storage.googleapis.com/fuze-public/widget-icons/twitch-vip.png',
        'prime': 'https://storage.googleapis.com/fuze-public/widget-icons/twitch-prime.png',
        'turbo': 'https://storage.googleapis.com/fuze-public/widget-icons/twitch-turbo.png',
        'bits': 'https://storage.googleapis.com/fuze-public/widget-icons/twitch-bits.png',
        'sub_gifter': 'https://storage.googleapis.com/fuze-public/widget-icons/twitch-sub-gifter.png',
        'partner': 'https://storage.googleapis.com/fuze-public/widget-icons/twitch-partner.png',
        'ambassador': 'https://storage.googleapis.com/fuze-public/widget-icons/twitch-ambassador.png',
        'artist': 'https://storage.googleapis.com/fuze-public/widget-icons/twitch-artist.png',
        'staff': 'https://storage.googleapis.com/fuze-public/widget-icons/twitch-staff.png',
    }},
    youtube: {{
        'moderator': 'https://storage.googleapis.com/fuze-public/widget-icons/youtube-moderator.png',
        'member': 'https://storage.googleapis.com/fuze-public/widget-icons/youtube-member.png',
        'coin': 'https://storage.googleapis.com/fuze-public/widget-icons/youtube-coin.png',
        'verified': 'https://storage.googleapis.com/fuze-public/widget-icons/youtube-verified.png',
        'owner': 'https://storage.googleapis.com/fuze-public/widget-icons/youtube-owner.png',
    }},
    kick: {{
        'broadcaster': 'https://storage.googleapis.com/fuze-public/widget-icons/kick-broadcaster.png',
        'moderator': 'https://storage.googleapis.com/fuze-public/widget-icons/kick-moderator.png',
        'vip': 'https://storage.googleapis.com/fuze-public/widget-icons/kick-vip.png',
        'og': 'https://storage.googleapis.com/fuze-public/widget-icons/kick-og.png',
        'founder': 'https://storage.googleapis.com/fuze-public/widget-icons/kick-founder.png',
        'subscriber': 'https://storage.googleapis.com/fuze-public/widget-icons/kick-subscriber.png',
        'sub_gifter': 'https://storage.googleapis.com/fuze-public/widget-icons/kick-sub-gifter.png',
        'staff': 'https://storage.googleapis.com/fuze-public/widget-icons/kick-staff.png',
        'verified': 'https://storage.googleapis.com/fuze-public/widget-icons/kick-verified.png',
    }},
    facebook: {{
        'moderator': 'https://storage.googleapis.com/fuze-public/widget-icons/facebook-moderator.png',
        'subscriber': 'https://storage.googleapis.com/fuze-public/widget-icons/facebook-subscriber.png',
        'top_fan': 'https://storage.googleapis.com/fuze-public/widget-icons/facebook-top-fan.png',
        'verified': 'https://storage.googleapis.com/fuze-public/widget-icons/facebook-verified.png',
    }},
    tiktok: {{
        'moderator': 'https://storage.googleapis.com/fuze-public/widget-icons/tiktok-moderator.png',
        'subscriber': 'https://storage.googleapis.com/fuze-public/widget-icons/tiktok-subscriber.png',
        'gifter': 'https://storage.googleapis.com/fuze-public/widget-icons/tiktok-gifter.png',
        'top_gifter': 'https://storage.googleapis.com/fuze-public/widget-icons/tiktok-top-gifter.png',
    }},
}};
...
function connectWS() {{
    if (config.show_twitch && connectedPlatforms.includes('twitch')) {{
        connections.push(createWS('wss://bomby.us/ws/fuze-alerts/{user_id}/twitch/?token={token}'));
    }}
    if (config.show_youtube && connectedPlatforms.includes('youtube')) {{
        connections.push(createWS('wss://bomby.us/ws/fuze-alerts/{user_id}/youtube/?token={token}'));
    }}
    if (config.show_kick && connectedPlatforms.includes('kick')) {{
        connections.push(createWS('wss://bomby.us/ws/fuze-alerts/{user_id}/kick/?token={token}'));
    }}
    if (config.show_facebook && connectedPlatforms.includes('facebook')) {{
        connections.push(createWS('wss://bomby.us/ws/fuze-alerts/{user_id}/facebook/?token={token}'));
    }}
    if (config.show_tiktok && connectedPlatforms.includes('tiktok')) {{
        connections.push(createWS('wss://bomby.us/ws/fuze-alerts/{user_id}/tiktok/?token={token}'));
    }}
    // Always connect to donations channel
    if (config.show_donations !== false) {{
        connections.push(createWS('wss://bomby.us/ws/fuze-donations/{user_id}/?token={token}'));
    }}
}}
...
function addEvent(data) {{
    ...
    // Donations bypass platform filter (show if donations enabled)
    if (platform === 'donation') {{
        if (config.show_donations === false) return;
    }} else {{
...
function connectGoalWebSockets() {{
    ...
    connectWebSocket(`wss://bomby.us/ws/fuze-goals/${{userId}}/?token=${{widgetToken}}`, (e) => {{
        const data = JSON.parse(e.data);
        if (data.type === 'refresh') {{
            window.location.reload();
            return;
    ...
    if (goalType === 'tip') {{
        connectWebSocket(`wss://bomby.us/ws/fuze-donations/${{userId}}/?token=${{widgetToken}}`, (e) => {{
            const data = JSON.parse(e.data);
    ...
    platforms.forEach(platform => {{
        ...
        if (connectedPlatforms.includes(platform)) {{
            connectWebSocket(`wss://bomby.us/ws/fuze-alerts/${{userId}}/${{platform}}/?token=${{widgetToken}}`, (e) => {{
                const data = JSON.parse(e.data);
...
function saveData() {{
    fetch(`https://bomby.us/fuze/labels/save/${{userId}}?token=${{widgetToken}}`, {{
        method: 'POST',
...
function handleEvent(data) {{
    ...
    switch(eventType) {{
        ...
        case 'donation':
        case 'tip':
            // Get amount from raw_amount or parse from amount string
            const rawAmt = eventData.raw_amount ?? eventData.amount;
            const donationAmount = typeof rawAmt === 'number' ? rawAmt : (parseFloat(String(rawAmt).replace(/[^0-9.]/g, '')) || 0);
...
function connectWebSockets() {{
    ...
    connectWebSocket(`wss://bomby.us/ws/fuze-labels/${{userId}}/?token=${{widgetToken}}`);
    
    const platforms = LABEL_PLATFORMS[labelType] || [];
    if (platforms.includes('all')) {{
        // Also connect to donations channel for donation-related labels
        connectWebSocket(`wss://bomby.us/ws/fuze-donations/${{userId}}/?token=${{widgetToken}}`);
    }}
    
    // Connect to platform alerts for platform-specific labels
    platforms.forEach(platform => {{
        if (platform !== 'all' && connectedPlatforms.includes(platform)) {{
            connectWebSocket(`wss://bomby.us/ws/fuze-alerts/${{userId}}/${{platform}}/?token=${{widgetToken}}`);
        }}
    }});
}}
...
fetch(`https://bomby.us/fuze/labels/data/${{userId}}?token=${{widgetToken}}`)
    .then(r => r.json())
    .then(res => {{
...
async function pollTwitch() {{
    ...
    try {{
        const resp = await fetch(`https://bomby.us/fuze/viewers/twitch/${{userId}}?token=${{widgetToken}}`);
        if (resp.ok) {{
...
async function pollYouTube() {{
    ...
    try {{
        const resp = await fetch(`https://bomby.us/fuze/viewers/youtube/${{userId}}?token=${{widgetToken}}`);
        if (resp.ok) {{
            const data = await resp.json();
...
async function pollKick() {{
    ...
    try {{
        const resp = await fetch(`https://bomby.us/fuze/viewers/kick/${{userId}}?token=${{widgetToken}}`);
        if (resp.ok) {{
            const data = await resp.json();
...
async function pollFacebook() {{
    ...
    try {{
        const resp = await fetch(`https://bomby.us/fuze/viewers/facebook/${{userId}}?token=${{widgetToken}}`);
        if (resp.ok) {{
            const data = await resp.json();
...
async function pollTiktok() {{
    ...
    try {{
        const resp = await fetch(`https://bomby.us/fuze/viewers/tiktok/${{userId}}?token=${{widgetToken}}`);
        if (resp.ok) {{
            const data = await resp.json();
...
function connectWS() {{
    const ws = new WebSocket(`wss://bomby.us/ws/fuze-viewers/${{userId}}/?token=${{widgetToken}}`);
...
function init() {{
    ...
    images = [];
    const defaultImage = 'https://storage.googleapis.com/fuze-public/media/sponsor-default.png';
...
function connectWS() {{
    const ws = new WebSocket(`wss://bomby.us/ws/fuze-sponsor/${{userId}}/?token=${{widgetToken}}`);
    ws.onmessage = (e) => {{
        const data = JSON.parse(e.data);
