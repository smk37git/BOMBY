...
def _verify_token(token):
    secret = os.environ.get('FUZE_SECRET_KEY', os.environ.get('DJANGO_SECRET_KEY', '')).encode()
    try:
        parts = token.split(':')
        if len(parts) != 5:
            return None
        ...
        if getattr(user, 'fuze_token_version', 0) != int(token_version):
            return None
        return user
    except Exception:
        return None
...
@csrf_exempt
def fuze_leaderboard(request, period='all'):
...
@csrf_exempt
def fuze_leaderboard_optin(request):
...
@csrf_exempt
def fuze_leaderboard_optout(request):
...
@csrf_exempt
def fuze_leaderboard_sync(request):
...
@csrf_exempt
def fuze_leaderboard_cron_sync(request):
